...
- Version/build authority is `Resources/Info.plist` plus `scripts/verify_app.sh`. Version 1.3.3 build 25 passed 202 tests, release-script regressions, and signed universal bundle verification on 2026-09-23. This follow-up changes presentation only; strict-concurrency and sanitizer suites were not repeated. Install by replacing the whole bundle so its outer directory timestamp reflects the new build.
- The user's build-24 screenshot exposed duplicate left/right checks despite `showsStateColumn = false`. Build 25 leaves main-menu toggle item state off and uses only the trailing badge plus Enabled/Disabled tooltip; logical settings and native actions are unchanged. The enabled-toggle regression checks both absence of native checked state and presence of the trailing badge. Live accessory-menu capture still times out, so exact final screen alignment is not visually verified. Do not infer that a hidden state-column flag alone removes checked-item indentation.
